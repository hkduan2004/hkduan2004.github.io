...
window.onload = function() {
	...
	setTimeout(function() {
		$(".kv_slide1").eq(0).addClass("kv_active").siblings().removeClass("kv_active");
	}, 1200);
...
//		initialSlide :4,
		...
		onSlideChangeEnd: function(swiper) { //slider切换结束时执行。 
			...
			if(swiper.activeIndex != 0) {
				...
				document.querySelector(".kv_swiper .kv_video_1").volume = 0;
				document.querySelectorAll(".kv_swiper .kv_video")[0].volume = 0;
				document.querySelectorAll(".kv_swiper .kv_video")[1].volume = 0;
				SAM.isF = false;
			} else {
				SAM.isF = true;
				document.querySelector(".kv_swiper .kv_video_1").volume = 1;
				document.querySelectorAll(".kv_swiper .kv_video")[0].volume = 1;
				document.querySelectorAll(".kv_swiper .kv_video")[1].volume = 1;
	...
	var kvSwiper = new Swiper('#kv_swiper', {
		...
		onSlideChangeEnd: function(swiper) {
			document.querySelectorAll(".kv_video_1").currentTime = 0;
			document.querySelectorAll(".kv_video")[0].currentTime = 0;
			document.querySelectorAll(".kv_video")[1].currentTime = 0;
			...
			if(swiper.activeIndex == 4 || swiper.activeIndex == 0) {
				if(SAM.isF) {
					swiper.stopAutoplay();
					kv_timer2 = setTimeout(function () {
						swiper.startAutoplay();
					},15000)
					document.querySelector(".kv_swiper .swiper-slide-active .kv_video").play();
				}
			} else {
				
				clearTimeout(kv_timer2);
				document.querySelectorAll(".kv_video")[0].pause();
				document.querySelectorAll(".kv_video")[1].pause();
			}
			if (swiper.activeIndex == 3) {
				if(SAM.isF) {
					swiper.stopAutoplay();
					kv_timer1 = setTimeout(function () {
						swiper.startAutoplay();
					},30000)
					document.querySelector(".kv_video_1").play();
				}
			}else {
				document.querySelector(".kv_video_1").pause();
				clearTimeout(kv_timer1);
				
			}
			if (swiper.activeIndex == 4 && swiper.activeIndex == 0 && swiper.activeIndex == 3) {
				swiper.startAutoplay();
			}
...
//			$(".canvas_div").addClass("can_active");
	...
	$('.play_icon1').on("click", function() {
		$('.play_icon1').hide();
		document.querySelector(".kv_swiper .swiper-slide-active .kv_video").play();
	});
	document.querySelectorAll(".kv_video")[0].addEventListener("timeupdate", function() {
		var _this = this;
		if(_this.currentTime > 1) {
			$(".play_icon").hide();
		}
	});
	document.querySelectorAll(".kv_video")[1].addEventListener("timeupdate", function() {
		var _this = this;
		if(_this.currentTime > 1) {
			$(".play_icon").hide();
		}
	});
	document.querySelector(".kv_video_1").addEventListener("timeupdate", function() {
		var _this = this;
		if(_this.currentTime > 1) {
			$(".play_icon").hide();
		}
